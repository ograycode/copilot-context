...
use crate::config::{match_files_and_mark, parse_file_rules};
...
pub fn clean_context_folder(
    ...
    std::fs::create_dir_all(dest).map_err(|e| {
        format!("Failed to create destination directory '{}': {}", dest, e)
    })?;
    ...
    for source in sources {
        match source {
            crate::config::Source::Repo { dest: src_dest, files, .. } |
            crate::config::Source::Url { dest: src_dest, files, .. } |
            crate::config::Source::Path { dest: src_dest, files, .. } => {
                // Add the destination path to the keep list
                let full_dest = Path::new(dest).join(src_dest);
                keep_files.insert(full_dest.clone());
                
                // Keep all files in the destination directory by default
                // if no specific file rules are provided
                if files.is_none() {
                    // Add all files in the destination directory to the keep list
                    if full_dest.exists() {
                        for entry in WalkDir::new(&full_dest)
                            .into_iter()
    ...
    fn test_clean_command_preserves_empty_directories() {
        ...
        if remove_dir.exists() {
            let is_empty = fs::read_dir(&remove_dir)
                .map(|entries| entries.count() == 0)
                .unwrap_or(false);
            assert!(is_empty, "Remove directory should be empty if it still exists");
        }
    }
} 